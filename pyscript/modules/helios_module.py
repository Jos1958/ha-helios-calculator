...
# Module: HELIOS Optimize Energy Service                            Version: 0.6
...
#   2026-08-01: JR: v0.6 Added Solar optimization (enabled, modulating, binary, disabled)
#
# Description:
...
def helios_optimize_energy(
    ...
    solar_mode     = solar.get("mode", "all").lower() # Mode: all, modulating, binary
    ...
    if solar_enabled: # solar applicable?
        ...
        if solar_mode == "binary":
            active_vars.extend("solar_switch") # add solar turn on/off variable type (binary)
    if ev_enabled:
        active_vars.append("ev_charge") # add solar charge variable type
    if hp_enabled:
        active_vars.append("hp_power") # add heatpump power variable type
    if boiler_enabled:
        active_vars.append("boiler_power") # add boiler power variable type
    ...
    # -------------------------------------------------------------------
    # 3. COST VECTOR (c), BOUNDS and INTEGRALITY are determined
    # -------------------------------------------------------------------
    c = np.zeros(total_vars) # factor per optimization variable (default factor is 0)
    bounds = [(0, 0)] * total_vars
    integrality = np.zeros(total_vars)  # variable is 0 = continuous, 1 = on or off (binary)
    ...
    for t in range(T):
        ...
        if solar_enabled:
            ...
            sol_pw_min = 0
            sol_pw_max = float(solar_forecast[t])
            if solar_mode == 'modulating': # Dim possible
                sol_pw_min = 0 # 0 <= solar_prod <= solar_forecast
            elif solar_mode == 'binary':
                idx_ss = get_idx(t, "solar_switch") 
                sol_pw_min = 0  # solar_prod = 0 or solar_forecast
                milp_enabled = True # MILP optimizer required
                integrality[idx_ss] = 1 # Enable integrality for solar switch      
            else: # no dimming and no switch off ("all")
                sol_pw_min = sol_pw_max # solar_prod = solar_forecast
            bounds[idx_sp] = (sol_pw_min, sol_pw_max)
